  ...
  return (
    <div className="fixed z-50 w-full bg-black border-b-2 border-blue-500 shadow-md md:relative md:border-b-0">
      <div className="container flex items-center justify-between py-4 mx-auto ">
        ...
        <button
          className="flex flex-col justify-center w-10 h-10 md:hidden pr-16"
          onClick={() => setMenuOpen(!menuOpen)}
        >
          <span className={`block h-1 bg-white rounded-lg transition-all duration-300 ${menuOpen ? "rotate-45 translate-y-0.5 w-full pr-16" : "w-10 mb-1"}`}></span>
          <span className={`block h-1 bg-white rounded-lg transition-all duration-300 ${menuOpen ? "-rotate-45 -translate-y-0.5 w-full pr-16" : "w-6"}`}></span>
        </button>
      </div>

      <div
        className={`md:hidden fixed left-0 w-full bg-black overflow-auto transition-all duration-300 ${
          menuOpen ? "h-[65lvh] py-4 border-b-2 border-blue-500" : "h-0 py-0"
        }`}
        style={{ zIndex: 999 }}
      >
        <ul className="flex flex-col items-center pl-2 text-4xl gap-3">
          {menuItems.map((item) => (
            <li key={item.path}>
              <Link
                href={item.path}
